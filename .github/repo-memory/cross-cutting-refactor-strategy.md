...
4. **Per-PR gates** for a cross-cutting refactor: `npx tsc --noEmit` + `npm run build` + `npx vitest run` (non-interactive; bare `npm test` is watch mode) + `npm run test:e2e` (which fans out to `pretest:e2e` → build + test-world JSON + `setup:e2e` → Playwright). The refactor is mechanical, so the only way regressions surface is in E2E — it runs per PR, not per story. (This is the exception to the project's general "E2E every story, not every PR" rule, which applies to feature-flavored phase work.) Doc-only PRs may skip the build and E2E gates.
