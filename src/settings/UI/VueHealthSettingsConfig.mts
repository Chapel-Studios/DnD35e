 ...
 * Uses Vue for rendering instead of Handlebars
 */

import type { HealthConfig } from '@settings/_types.mjs';
import { DEFAULT_HEALTH_CONFIG, SETTINGS, SYSTEM_ID } from '@settings/constants/index.mjs';
import { HealthSettingsApp } from '@vc/index.mjs';
 ...
 */
class VueHealthSettingsConfig extends VueSettingsBase {
  ...
  constructor(...args: any[]) {
    ...
    const currentConfig = game.settings.get(SYSTEM_ID, SETTINGS.HEALTH_CONFIG) as HealthConfig;
  ...
  /**
   * Create the Vue app with event handlers
   */
  protected override _createVueApp(renderOptions: VueSettingsRenderOptions) {
    const app = super._createVueApp(renderOptions);

    // The component emits 'submit' and 'reset' events via the root
    // We handle them by passing callbacks via props in the base mixin
    return app;
  }
  ...
  async #onSave(): Promise<void> {
    ...
    try {
      await game.settings.set(SYSTEM_ID, SETTINGS.HEALTH_CONFIG, data);
...
export { VueHealthSettingsConfig };
