...
- [x] Create `ActionChainLinkModel` — implemented as a plain TS type (`ActionSourceData.mts`) backing an inline `ArrayField(SchemaField)` `chain` field on `ActionDataModel`, rather than a standalone `DataModel` subclass
- [x] Create `ActionDataModel` base schema: `activationCost`, `chain`, `provokes`, `maxTargets` (initial 1), `isTargetRequired`, `isSystemCreated`, `isTopLevel` — no `hand`/`defaultSlotId` fields (both derived live at execution time, §10.3). **Deviation**: the doc's nested `check`/`damage` schema groups were flattened directly onto `WeaponAttackDataModel` (`attackFormula`, `damageFormula`, `damageType`, `critRange`, `critMultiplier`, `requiresEquipped`) instead of nested objects on the base — `range` stays deferred to Story F
- [x] Declare the public `executeAction()`/`continue()` interface methods on `ActionDataModel` (§10.7) — Story C ships functional-but-partial bodies (target-count validation, melee reach validation, then an early `cancelled: true` return); Story D fills in the Attack Roll Dialog/roll/chat-card/chain-advance work
- [x] Create `MeleeWeaponAttack`/`RangedWeaponAttack` subtypes and wire `actions` on `WeaponSystemModel` as `ArrayField(TypedSchemaField({melee_weapon_attack, ranged_weapon_attack}))` (§10.3). **Deviation**: no separate `ThrownWeaponAttack` subtype — thrown is instead a `WEAPON_PROPERTY_THROWN` flag on `RangedWeaponAttack.properties` (extending §10.4's own "thrown is an orthogonal detail, not a category" reasoning one step further than the doc drafted it)
- [x] Add `twoHandedRanged` to `WEAPON_SUBTYPES`/`WEAPON_SUBTYPE`/`WEAPON_SUBTYPE_LOCALIZED` (`constants.mts`) — drives the Melee⇄Ranged auto-swap below alongside the existing `ranged` value
- [x] Attach FormulaFamiliar contexts to `attackFormula`/`damageFormula` (the doc's `check.formula`/`damage.formula`, per the flattened-schema deviation above)
- [x] Implement `weaponActionSync.mts` (sibling of `masterworkAe.mts`'s `syncMasterworkAeState()`): auto-create/delete `MeleeWeaponAttack`↔`RangedWeaponAttack` on `weaponSubtype` ranged-group changes, deleting only entries still `isSystemCreated === true`; wired from `Weapon._onUpdate()`/`_onCreate()`, not `prepareDerivedData()`. No separate thrown-toggle sync needed — `thrown` is a `properties` flag on the existing entry, not a synced third entry
- [x] Implement `EquippableItem._buildEquippedChanges()` and `Weapon.getContributedActorChanges()`: live-merge each system-created, still-unedited action stub onto `system.actions.<id>` via `ActionDataModel.createActionChange()`
- [x] Implement `getTwoWeaponFightingPenalty()`: -6/-10 baseline, -4/-8 on both hands when the **off-hand's** currently-equipped weapon has `weaponSubtype === LIGHT_WEAPON` (not the attacking hand's weapon)
- [x] Add `$floor`/`$ceiling`/`$round`/`$absolute` (plus `$scaleDamage`, see below) to `FormulaResolver.functionGrammar.mts` (§10.3) — new `FunctionName` entries, single-arg; resolve the inner argument and compute the result locally (`Math.floor`/`Math.ceil`/`Math.round`/`Math.abs`) whenever it's already a plain number, falling back to re-emitting `$`-stripped Math-proxy syntax (`$ceiling` → `ceil`, `$absolute` → `abs`) only when real dice-term syntax remains for Foundry's native Roll grammar to evaluate later
- [x] Create weapon-property constants (`WEAPON_PROPERTY_*`/`WEAPON_PROPERTIES`) and `properties` `SetField`s; expose as a plain checkbox list on the weapon sheet. **Deviation**: properties live split across `WeaponAttack/constants.mts` (base: `nonLethal`/`nonLethalNoPenalty`) plus per-subtype extensions (`MeleeWeaponAttack/constants.mts`: `finesse`/`reach`/`threatensAdjacent`; `RangedAttack/constants.mts`: `thrown`/`rangedUsesStr`) on each **action**, not a single flat array on `WeaponSystemModel`
- [x] Implement `detectWieldMode(actor, item)`, `getWieldModeStrTerm(wieldMode)`, and `wieldModeToBabHand(wieldMode)` (`ActorDnd35e.mts`, §10.3) — the contributed default action's `damageFormula` never bakes in a flat `#self.abilities.str.mod`, and the same wield-mode detection now also decides which BAB pool(s) an attack draws from (no stored `hand` field)
- [x] Build the Actions tab on the weapon sheet (`WeaponActionsTab.vue` + shared `ActionsEditorList.vue`/`ActionAccordionRow.vue`): an editable list of the system-managed actions, plus "add to chain" sub-action authoring (a superset of this phase's own no-custom-actions scope — the chain-authoring UI is generic and already usable, custom **top-level** action authoring stays alpha.3)
- [x] Implement the `itemEquipped`/`itemUnequipped` listener on `equipped.mts` that spends a move action via `spendAction()` when equipping/unequipping mid-combat (gated on `game.combat?.started`) — a separate, always-on mechanic, never triggered by using a `requiresEquipped: false` action. (The Hand-select dialog field itself is Story D scope — the Attack Roll Dialog doesn't exist until then; still listed there)
- [x] Change `weaponDamage.critRange`/`WeaponAttackDataModel.critRange` from `StringField` to `NumberField` (integer, min 2, max 20, initial 20); add `formatThreatRange()` display helper (`threatRange.mts`, §10.4 "Threat range becomes a number"); change `D20Roll.isCriticalThreat` from a no-arg getter to `isCriticalThreat(threshold: number = 20)`
- [x] Default `designedForSize` to the parent actor's own `system.size` via `EquippableItem._preCreate()` when embedded directly on an Actor and no size was explicitly provided (§10.4 "Weapon damage scaling by size") — generalized to all `EquippableItem`s, not weapon-specific
- [x] Implement `scaleDamageDie(dieFormula, size)` as a plain TS helper (`weaponDamageScaling.mts`, §10.4) — transcribed from D35E's own size-die step table. **Deviation**: rather than a `weaponDamage.autoScaleDamage` boolean field with `Weapon.getContributedActorChanges()` calling it directly, scaling is invoked live via a new `$scaleDamage(dieFormula)` FormulaFamiliar function (`FormulaResolver.functionGrammar.mts`) embedded directly in `damageFormula`'s initial value (`$scaleDamage(1d4)`) — removing the function call from the formula is the opt-out, instead of a separate toggle field

**Verify**: Create a weapon → `system.actions` auto-seeds a `MeleeWeaponAttack` (or `RangedWeaponAttack`, per `weaponSubtype`); checking the `thrown` property flag doesn't change which entry exists (it's a property on the existing `RangedWeaponAttack`, not a separate synced entry). Switching a weapon's `weaponSubtype` between the ranged group and everything else swaps `MeleeWeaponAttack`↔`RangedWeaponAttack` (deleting the inapplicable one only if still system-created). FormulaFamiliar autocomplete includes `#self.abilities.str.mod`, `$floor`/`$ceiling`/`$round`/`$absolute`, and `$scaleDamage`. Editing a system-created action's field flips its `isSystemCreated` badge off. Actions tab lists each system-managed action as an editable row, with working "add to chain" sub-action authoring. `critRange` renders as `20`/`19 / 20`/`17-20` per `formatThreatRange()`'s convention. Dragging a compendium weapon onto a Large actor defaults `designedForSize` to `large`; a `damageFormula` containing `$scaleDamage(...)` reflects the Large-scaled die at resolution time, not the entered Medium baseline. Equipping/unequipping a weapon mid-combat spends a move action.

**Remaining for a later story** (moved to Story D, since both are HUD/Actions-tab-triggered UI, not schema/sync work): the generic always-available "Throw" action, and the Attack Roll Dialog's Hand select field.
...
- [x] Build new bottom-row Token HUD control with two entries, **Weapon Attacks** and **Combat Maneuvers**, each expanding a list on click (same interaction pattern as the existing Movement Action control, not a shared button); Weapon Attacks eligibility per entry `item.system.isCarried && (item.system.isEquipped || action.requiresEquipped === false)`, enabled state `canUseHandAttack(combatant, wieldModeToBabHand(detectWieldMode(actor, item)))`; Combat Maneuvers renders a single Total Defense entry this phase (wired by Story E) — alpha.3 adds real maneuvers to the same list, no new HUD control needed then. **Deviation**: shipped as a full Vue takeover of core's `TokenHUD` (`TokenHudDnd35e.mts`, `TokenHudApp.vue`, mirroring the `CombatTrackerDnd35e` precedent) rather than a Handlebars-subclass decoration, per explicit direction to keep the whole system Vue-first — every native control (elevation, bars, visibility, lock, sort, config, status effects, movement actions, levels, target, combat) is faithfully reproduced with the same classes/`data-action` attributes so `BasePlaceableHUD`'s/`TokenHUD`'s native click handling and `movementActionHudDecoration.mts`'s `renderTokenHUD` hook keep working unmodified. The Combat Maneuvers entry is a UI shell only this story — clicking it shows a "not yet implemented" notice until Story E wires the real Total Defense mechanic (spend standard action + apply the AE).
- [x] Repurpose `CombatTab.vue`/`combatTab` into the Actions tab (id/label/tooltip rename); delete `AttackBonusSection.vue` entirely (CMB/CMD isn't a 3.5e concept — see §10.11)
- [x] Build out `WeaponsSection.vue` for real: one row per carried weapon action with resolved attack/damage/crit/range/type, plus the carried-gated attack button (works regardless of equip state, unlike the HUD)
- [x] Create `DamageRoll` (`src/dice/DamageRoll.mts`, sibling of `D20Roll`; pushed back from Phase 7)
- [x] Register `preUseAction`, `postUseAction`, `dealDamage`, `undoDealDamage` in `wellKnownEvents`
- [x] Define `UseActionContext` (incl. `hand`, `free`) and `ActionResult` — deferred to D.3 alongside `executeAction()`/`useAction()`, whose concrete needs determine the exact shape. **Deviation**: split per-action-kind instead of one flat shape — base `ActionResult` (`cancelled`/`reason`/`warnings`) plus `WeaponAttackActionResult extends ActionResult` (`attackTotal`/`nonLethal`/`attackMessage`); `ActionDataModel` is generic over its own result type (`TResult extends ActionResult`) so future action kinds declare their own result shape instead of piling fields onto one shared type.
- [x] Implement `actor.useAction()`: `preUseAction` cancellation, `canUseHandAttack()` gate (bypassed when `free: true`), spends standard action + 5 BAB from the acting hand; after a charge's mandatory attack resolves, calls `markMovedAfterAttack()` when `used.chargedThisTurn` is set (§10.1/§10.6, SRD's "single melee attack" restriction) rather than adding a second gating flag. **Deviation**: shipped as `Creature.useAction()` (not on the actor base) — wiring the HUD/sheet triggers to it is left to this story's still-open HUD/Actions-tab items above. **Follow-up fix**: `useAction()` takes `targetTokens?: TokenDnd35e[]` (not ids or resolved Actors) so an unlinked token's target always resolves to its own synthetic Actor — `game.actors.get(id)` always returned the shared prototype Actor, which broke targeting when multiple unlinked copies of the same Actor were on the scene.
- [x] Extend `D20RollDialogData`/`Result` with the optional `combatModifiers` checklist, split into Attack Type (Charge/Defensive Fighting/Non-lethal) and Combat Status (Flanking/High Ground/Squeezing/Prone/Proficient) sections, each toggle rendering a help-icon tooltip explaining its effect, plus a separate `damageBonus` formula field — D.2, complete. **Scope note**: `isFlanking()`'s real implementation is a Story G deliverable that itself depends on Story D, creating a circular ordering; resolved by shipping Flanking as a manual-only toggle in Story D (`autoDetected: false`), with real geometry auto-detection deferred entirely to Story G.
- [x] Wire `isOnHigherGround()`/`actor.statuses.has('prone')`/`actor.statuses.has('squeezing')`/the combatant's `used.chargedThisTurn` flag auto-detection into `executeAction()`'s dialog call (D.3); Non-lethal's `checked` default comes from `item.system.properties.has(WEAPON_PROPERTY_NON_LETHAL)` (§10.3). Flanking is manual-only per the scope note above — no auto-detection wired for it in Story D. Shipped as `createCombatModifiers()` (`WeaponAttack/combatModifiers.mts`).
- [x] Reserve an Ammo select field slot in the dialog, shown only when the acting action has a non-null `range.ammoType` — populated by Story F
- [x] Add a Wield Mode `MultiOptionToggle` field (Primary Hand / Off-Hand / Two-Handed, §10.3) to the Attack Roll Dialog, pre-selected by `detectWieldMode(actor, item)` from the weapon's current `equippedSlotIds` but always user-overridable
- [x] Implement `ActionDataModel#executeAction()` (`ActionDataModel.mts`, Story C's file) — the public interface method `useAction()` calls on the action itself: accepts `targetIds: string[]`, truncated to `this.maxTargets` (§10.3 — always 1 for poc.10's weapon actions) with a warning if the caller offered more; open the Attack Roll Dialog (auto-detected + manual combat modifiers, roll mode), resolve `check`/`damage` formulas via `FormulaData.resolve()` (never `getRollData()`), reach check per target (melee only, doubled when the weapon has the `reach` property but excluding the attacker's own un-doubled reach unless `threatensAdjacent` is also set — spiked chain and homebrew equivalents opt out of the reach-weapon adjacent-foe dead zone, §10.3), TWF penalty applied to the resolved formula, D20Roll for the attack total — **no hit/miss decision and no damage roll here**; snapshot the resolved (unrolled) damage formula + damage bonus term + crit multiplier per target. **Deviation**: `Creature.useAction()` passes a single `targetId?: string` (not an array), so the truncation warning is currently unreachable dead code — revisit once/if multi-target selection is wired. **Follow-up fix**: reach/flanking/high-ground checks (`createCombatModifiers()`) take the target's own `TokenDnd35e` object threaded straight through from `Creature.useAction()` → `PrepareActionContext()` → `UseWeaponAttackContext.targetToken`, rather than re-deriving "the" token from a resolved target Actor — Actor → Token is ambiguous whenever multiple unlinked tokens share one prototype Actor, while Token → Actor is always lossless, so the Token is what gets threaded.
- [x] Apply the Charge/Defensive Fighting self-AE (`applyChargedAE()`/`applyDefensiveFightingAE()`, Story E) immediately once the dialog confirms those toggles were checked
- [x] Compute the non-lethal switch penalty (§10.3): −4 only when the dialog's final Non-lethal checked state differs from `item.system.properties.has(WEAPON_PROPERTY_NON_LETHAL)`, waived when `item.system.properties.has(WEAPON_PROPERTY_NON_LETHAL_NO_PENALTY)`; applied alongside `twfPenalty`, not folded into the toggle's own value
- [x] Implement `getWieldModeStrTerm(wieldMode)` and append it to the resolved damage formula (full STR / `$floor(str/2)` / `$floor(str*1.5)` per the dialog's Wield Mode selection, §10.3) — for the ranged/thrown default action, only appended when `item.system.properties.has(WEAPON_PROPERTY_THROWN)` (true ranged weapons don't add STR to damage)
- [x] Thread the Non-lethal toggle's checked state through to `result.nonLethal`, carried on the attack card's flags for Story E's eventual resolution/damage step to read
- [x] Add `buildAttackCard()` to `src/dice/rollMessages.mts` (precompiled `.hbs?raw` + `Handlebars.compile()`, reusing `appendModifierBreakdown()`), storing `flags.dnd35e.attackCard` with one row per target. **Deviation**: shipped as `src/dice/chatCards/attackRollCard/attackRollCard.mts` per the dice/ folder restructure (chatCards/ split), reusing `buildDiceRollHtmlWithModifiers()` (renamed from `appendModifierBreakdown()`).
- [x] Create `src/dice/templates/attack-roll-card.hbs` (per-target rows, Apply per row, Change Target(s) button). **Deviation**: lives at `src/dice/chatCards/attackRollCard/attack-roll-card.hbs`. Apply-per-row button is Story E scope (hit resolution) — only Change Target(s) is wired this story.
- [x] Register the `renderChatMessageHTML` hook's delegated `data-action="retarget"` click handler (GM-only): re-syncs unresolved attack-card rows to `game.user.targets`. Shipped as `wireAttackRollCardButton()`/`onRetarget()`, gated to GM or the attacker's owner (not GM-only as originally scoped — matches the same owner-or-GM gating used elsewhere on this card). **Follow-up fix**: every attacker/target name shown in the roll dialog and this card (including `onRetarget()`'s rebuilt rows) reads the placed Token's own `name` (`token.name` / `actor.token?.name ?? actor.name`), not the shared Actor's `name` — unlinked tokens sharing one Actor can have distinct display names. Also added `canUserSeeActorName()` (`tokenNameVisibility.mts`): the GM always sees real names; players see them per Foundry's own `CONST.TOKEN_DISPLAY_MODES` nameplate rules. Live dialogs gate on it directly; the chat card's `content` is one static HTML string shared by every viewer, so a `redactAttackCardNames()` pass re-runs the check per viewer at actual render time via the existing `renderChatMessageHTML` hook.
...
- [ ] Add the generic, always-available "Throw" HUD/Actions-tab action (§10.4, moved from Story D since it's a range-increment/ammo-flavored action in its own right): throws whatever's held in a free/freeable hand at a flat −4 attack penalty, 10-ft range increment, crit only on natural 20 (×2 damage) — used only when the thrown item has no real `RangedWeaponAttack` entry with the `thrown` property of its own
...
- [ ] Bug carried from Story D: `createCombatModifiers()` (`WeaponAttack/combatModifiers.mts`) auto-checks `prone` (line ~37) and `highGround` (line ~55) unconditionally for every attack, regardless of melee vs. ranged. SRD's Prone −4 only applies to melee attack rolls (ranged while prone is disallowed instead, except crossbows — §10.7's table, also unenforced); confirm whether High Ground should likewise be gated by attack type before fixing. Needs the acting action's melee/ranged kind threaded into `createCombatModifiers()`.
