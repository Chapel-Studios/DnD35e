...
- [ ] Add the generic, always-available "Throw" HUD/Actions-tab action (§10.4, moved from Story C): throws whatever's held in a free/freeable hand at a flat −4 attack penalty, 10-ft range increment, crit only on natural 20 (×2 damage) — used only when the thrown item has no real `RangedWeaponAttack` entry with the `thrown` property of its own
...
- [ ] Define `UseActionContext` (incl. `hand`, `free`) and `ActionResult` — deferred to D.3 alongside `executeAction()`/`useAction()`, whose concrete needs determine the exact shape
...
- [ ] Wire `isOnHigherGround()`/`actor.statuses.has('prone')`/`actor.statuses.has('squeezing')`/the combatant's `used.chargedThisTurn` flag auto-detection into `executeAction()`'s dialog call (D.3); Non-lethal's `checked` default comes from `item.system.properties.has(WEAPON_PROPERTY_NON_LETHAL)` (§10.3). Flanking is manual-only per the scope note above — no auto-detection wired for it in Story D.
