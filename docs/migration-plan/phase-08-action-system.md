...
class TurnActionBudget {
  /** Current state of the turn's action economy */
  state: TurnState;

  /** Track what's been spent */
  spent: {
    standard: boolean;
    move: boolean;
    swift: boolean;
...
Each attack produces a **separate chat message**. This gives clear per-attack resolution and makes it easy to reference individual results.
