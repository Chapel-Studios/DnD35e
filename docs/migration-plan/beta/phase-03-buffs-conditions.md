...
├── bonusType: BonusType (for stacking — leverages Phase 2 infrastructure)
├── active: boolean
├── timeline.total: number (total rounds)
├── timeline.elapsed: number (rounds elapsed)
├── timeline.formula: string (duration formula)
├── timeline.deleteOnExpiry: boolean (auto-remove when expired?)
├── damagePool.max: number (max absorb, e.g. Stoneskin 150 HP)
├── damagePool.current: number (remaining absorb)
├── shapechange.type: string (polymorph/wildshape type)
├── hideFromToken: boolean (suppress icon on token)
└── changes: Dnd35eEffectChangeData[] (targeting actor or item)
```

When a spell creates a buff on a target, it creates a Buff AE directly on the target actor. Buff AEs appear in a dedicated "Buffs" section on the character sheet. Timeline ticks down via combat tracker integration (Phase 9).

Duration uses the custom `timeline` fields above (NOT Foundry's built-in `ActiveEffect.duration`) to support formula-driven durations and damage pools.
...
This phase expands the Phase 5 ability score model from basic `{ base, mod }` to the full model with damage, drain, penalties, and energy drain. Phase 5 only implements `abilities.*.base` and `abilities.*.mod` — all sub-fields below are deferred to this phase. See actor PropertyMap (`docs/architecture/property-maps/PropertyMap-Actors.md`) for the full field list.

```
CreatureSystemModel.abilities.[ability] (expanded from Phase 5)
├── base: number (from Phase 5)
├── damage: number (NEW — ability damage, recovers with rest/magic, stacking cumulative)
├── drain: number (NEW — ability drain, permanent until restoration magic)
├── penalty: number (NEW — temporary penalties from AE/conditions, not damage/drain)
├── userPenalty: number (NEW — user-applied modifier for GM overrides)
├── checkMod: number (NEW — misc modifier to ability checks only, e.g. racial +2 Perception)
├── total: number (derived: base - damage - drain - penalty - userPenalty)
└── mod: number (derived: Math.floor((total - 10) / 2)) — Phase 5 formula updated
```

```
CreatureSystemModel (additional field)
└── energyDrain: number (NEW — negative levels from energy drain effects)
```

Energy drain applies -1 per negative level to: attack rolls, saves, skill checks, effective level. At negative levels ≥ HD, creature dies. Implemented as AE changes targeting multiple fields.
...
| Expand | Actor ability score model — damage, drain, penalty, userPenalty, checkMod per ability |
| Expand | Actor model — add `energyDrain` (negative levels) NumberField |
...
**Fast Healing & Regeneration (SRD Special Abilities):**
- [ ] Implement Fast Healing buff type: creature regains X HP per round at start of turn
- [ ] Fast Healing automation: EffectTrigger on 'turnStart' event, applies healing formula
- [ ] Fast Healing does NOT prevent death from massive damage or HP reaching -10
- [ ] Implement Regeneration buff type: creature regains X HP per round, lethal damage converted to nonlethal
- [ ] Regeneration vulnerability: specific damage types bypass regeneration (e.g., fire for trolls)
- [ ] Regeneration bypass stored as ArrayField on the buff (damage types that deal lethal)
- [ ] Regeneration creature cannot die from HP damage unless killed by bypass damage type
- [ ] Test: Fast Healing 5 creature recovers 5 HP at turn start
- [ ] Test: Regeneration 5 creature converts lethal to nonlethal except fire/acid

**Disease Mechanic (SRD Environment):**
- [ ] Implement Disease condition type with: name, infection DC, incubation period, damage formula
- [ ] Disease incubation: after exposure, no effects for incubation period (1d3 days typical)
- [ ] Disease recurring save: Fort save vs infection DC each day after incubation
- [ ] Disease effect on failed save: apply ability damage per disease's damage formula
- [ ] Disease recovery: two consecutive successful saves = cured (remove condition AE)
- [ ] Common diseases from SRD: Blinding Sickness, Cackle Fever, Filth Fever, Mindfire, Mummy Rot, Red Ache, Shakes, Slimy Doom
- [ ] Test: Disease condition applies ability damage on failed daily save
- [ ] Test: Two consecutive saves cure disease
...
- [ ] Add to each ability:
  - `base` (NumberField, actual ability score value, e.g., 16) — already exists from Phase 5
  ...
  - `userPenalty` (NumberField, user-applied modifier for GM overrides, stacking)
  - `checkMod` (NumberField, misc modifier to ability checks only, e.g. racial +2 to specific checks)
  - `total` (computed, base - damage - drain - penalty - userPenalty)
  - `modifier` (computed, Math.floor((total - 10) / 2)) — update Phase 5 formula to use `total` instead of `base`
...
- [ ] Add `energyDrain` NumberField to CreatureSystemModel (default 0)
- [ ] Implement energy drain effects: each negative level applies -1 to attack rolls, saves, skill checks, and effective level
- [ ] Implement via AE changes targeting multiple fields simultaneously
- [ ] At negative levels ≥ HD: creature dies (display warning at threshold)
- [ ] Recovery: 24 hours later, Fortitude save DC 10 + attacker HD to remove each negative level
- [ ] If CON drain from energy drain reduces CON-based HP to 0, creature dies
- [ ] Test: Apply 2 energy drain → -2 to attacks, saves, skills, effective level
- [ ] Test: Energy drain ≥ character HD → death warning displayed
- [ ] Test: Fortitude save to recover negative levels
