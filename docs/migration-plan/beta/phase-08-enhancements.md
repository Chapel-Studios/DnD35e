...
> **D35E Reference**: D35E stores enhancements as `system.enhancements.items[]` — a raw JSON array of full item data snapshots inside the parent item. CRUD via `duplicate() → mutate → item.update()`. When a sub-item needs to be used, a transient `Item35E` instance is constructed with `new Item35E(data)` using a random ID. This bypasses Foundry’s EmbeddedDocument system entirely.
>
> **Our approach**: Enhancements are Active Effects on items, following the Material pattern from Phase 2. D35E’s JSON-array-of-item-data approach is replaced by Foundry’s native AE system with our stacking engine.

Both Equipment and Loot `canAcceptEffects: true` — mundane base items become magical through enhancements. A Cloak of Resistance is a mundane cloak (Equipment) with a resistance enhancement AE. A magical trinket is mundane Loot with an enhancement AE that grants actions.
...
**Per-Secret Reveal Hooks (from Phase 2 §2.12):**
- [ ] Allow each Secret AE to define a hook/event that fires when the GM reveals it
- [ ] Integrate with Spellcraft/Identify flow — hook fires after a successful identification check
