...
**Gaze Attack Subtype (SRD Special Abilities):**
- [ ] Add gaze attack support to AttackSystemModel: isGaze (BooleanField), gazeRange (NumberField, default 30ft)
- [ ] Gaze attack mechanic: affects all creatures within range that look at the gazer (passive)
- [ ] Active gaze: as standard action, force one target to save (even if averting eyes)
- [ ] Averting eyes: 50% chance to avoid gaze each round, attacker has concealment against averter
- [ ] Closing eyes: immune to gaze but effectively blinded (all opponents have total concealment)
- [ ] Reflecting gaze: mirror/polished shield — gazer must save vs own gaze if within range
- [ ] Gaze save: typically Fort or Will, DC = 10 + ½ HD + CHA mod (or as specified)
- [ ] Common gaze effects: petrification (medusa), death (bodak), charm (vampire)
- [ ] Test: Gaze attack forces saves on creatures in range
- [ ] Test: Averting eyes gives 50% miss chance
- [ ] Test: Active gaze works as standard action against one target
