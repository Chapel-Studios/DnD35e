...
> **Milestone**: Alpha  
> **Dependencies**: Phase 5, Phase 9 (Action System)  
...
### Alpha Paladin Feat Build

The level 5 Human Paladin selects all three test feats during level-up via the character-level feat milestone system (Phase 8 §8.5 step 6) and Human racial bonus feat (Phase 7):

| Level | Source | Feat | Prerequisite |
|-------|--------|------|-------------|
| 1 | Character feat (1st HD milestone) | Power Attack | STR 13 ✓ |
| 1 | Human racial bonus feat | Weapon Focus (Longsword) | BAB +1 ✓ |
| 3 | Character feat (3rd HD milestone) | Cleave | Power Attack ✓ |

This is the only valid assignment order: Cleave requires Power Attack, so Power Attack must be taken first. Weapon Focus requires BAB +1, which the Paladin has at level 1.
...
**Conditional bonuses** (e.g., "+2 attack when flanking") are regular AE changes with `phase: 'action.attack'` (or `action.save`, `action.check`) and a `condition` field — a formula familiar string that resolves to boolean with the roll context (e.g., `"#self.isFlanking"`). These are **not applied during data prep** — the action system collects them at roll time, evaluates conditions, surfaces applicable ones in the PreRollDialog, and applies them through the stacking engine.
...
- [ ] Support conditional bonus via action-phase changes: `phase: 'action.attack'` with `condition` formula familiar string (e.g. `"#self.isFlanking"`)
- [ ] Conditional changes are regular `Dnd35eEffectChangeData` entries — no separate `_conditionalBonuses` array
- [ ] Implement roll-time condition evaluation: action system resolves `condition` formula, surfaces applicable changes in PreRollDialog
...
- [ ] Test: Add Weapon Focus (Longsword) to Paladin actor
...
- [ ] Test: Paladin has Power Attack feat
...
- [ ] Test: Paladin without Power Attack → slider doesn't appear
...
- [ ] Test: Paladin with Cleave kills enemy
...
- [ ] Test: Paladin without Cleave → no free attack on kill
...
**Conditional Bonus Roll-Time Integration (Action Phases):**
- [ ] Modify PreRollDialog to collect action-phase changes from actor's applicable effects
- [ ] Evaluate each change's `condition` formula with roll context
- [ ] For changes where condition evaluates to true, auto-include in roll
- [ ] For changes where condition cannot be auto-evaluated (player knowledge required), show checkbox in PreRollDialog
- [ ] Track which conditionals were applied in roll data for chat card
- [ ] Display applied conditionals in chat card details
- [ ] Test: Feat with action.attack change, condition `"#self.isFlanking"`, value +2
- [ ] Test: Condition evaluates true → bonus applied automatically
- [ ] Test: Condition cannot be auto-evaluated → checkbox shown in PreRollDialog
- [ ] Test: Chat card shows applied conditional bonuses
...
- [ ] Integration test: Create Paladin with Weapon Focus (Longsword)
- [ ] Integration test: Roll attack with weapon → +1 bonus from feat visible in chat
- [ ] Integration test: Paladin with Power Attack → PreRollDialog shows PA slider
- [ ] Integration test: Use PA slider → attack & damage adjust correctly
- [ ] Integration test: Paladin with Cleave → kill enemy → free attack granted
