...
- **FormulaFormGroup** — contenteditable input with syntax highlighting, real-time validation, and a dropdown autocomplete menu
- **FamiliarDropdown** — reusable dropdown that renders filtered autocomplete suggestions with keyboard navigation
- **AspectPicker** — property path picker used in the Active Effect config to select change keys from the target schema rather than typing raw paths
